    ...
    if ($user) {
        ...
        try {
            ...
            if ($user['user_role'] == 'seller') {
                ...
                $stmt = $pdo->prepare("UPDATE notifications SET is_read = 1 WHERE user_id = ? ORDER BY id DESC LIMIT 1");
                $stmt->execute([$user['id']]);
            }
            ...
            $_SESSION['verification_success'] = "✓ Your email has been verified! You can now log in to your account.";

            // Redirect straight to login
            header('Location: http://localhost/beautymart/includes/login.php');
            exit;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { font-size: 28px; color: #2d3748; margin-bottom: 10px; font-family: 'Playfair Display', serif; }
        .message { margin: 20px 0; padding: 15px; border-radius: 8px; font-size: 15px; line-height: 1.5; }
        .message.error   { background: #fed7d7; color: #742a2a; border: 1px solid #fc8181; }
        .message.info    { background: #feebc8; color: #7b341e; border: 1px solid #fbd38d; }
        .button {
        ...
        .resend-link { margin-top: 20px; font-size: 14px; color: #718096; }
        .resend-link a { color: #667eea; text-decoration: none; font-weight: 600; }
...
<div class="container">
    ...
    <?php if ($message_type == 'error'): ?>
        ...
        <div class="resend-link">
            <p>Didn't receive the email? <a href="resend_verification.php">Resend verification email</a></p>
        </div>
        <a href="http://localhost/beautymart/includes/login.php" class="button">Back to Login</a>

    <?php else: ?>
        <div class="icon">ℹ️</div>
        <h1>Already Verified</h1>
        <div class="message info"><?= htmlspecialchars($message) ?></div>
        <a href="http://localhost/beautymart/includes/login.php" class="button">Go to Login</a>
    <?php endif; ?>
